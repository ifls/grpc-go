...
//
// All errors returned from ClientStream methods are compatible with the status package. stattus包
type ClientStream interface {
	...
	// 返回从服务器收到的header元数据, 如果有
	Header() (metadata.MD, error)
	// Trailer returns the trailer metadata尾部元数据 from the server, if there is any.
	// It must only be called after stream.CloseAndRecv has returned, or stream.Recv has returned a non-nil error (including io.EOF).
	// 如果recv 返回了一个错误，可以调用获取尾部数据
	Trailer() metadata.MD
	// CloseSend closes the send direction of the stream. 关闭写端
	// It closes the stream when non-nil error is met.
	// It is also not safe to call CloseSend concurrently with SendMsg. 不能并发
	CloseSend() error
	// Context returns the context for this stream.
	//
	// It should not be called until after Header or RecvMsg has returned.
	// Once called, subsequent client-side retries are disabled. 重试被禁止？？
	Context() context.Context
	// SendMsg is generally called by generated code.
	// On error, SendMsg aborts the stream.
	// If the error was generated by the client, the status is returned directly;
	...
	// but it is not safe to call RecvMsg on the same stream in different goroutines. 流之间互不影响
	RecvMsg(m interface{}) error
}
